...
class LDmodel():
	...
	def __init__(self, nx, ns, npcl, xvar=1.0):
		...
		init_cov_inv=np.dot(init_W.T, init_W)/(xvar**2)
		...
		self.cov_inv=T.dot(self.W.T, self.W)/(self.xvar**2)
		
		self.theano_rng = RandomStreams()
		
		self.params=				[self.W, self.M, self.ln_b]
		self.rel_lrates=np.asarray([  0.1,    1.0,     100.0]   ,dtype='float32')
		
		self.meta_params=     [self.C]
		self.meta_rel_lrates=[   1.0  ]
	
	
	def sample_proposal_s(self, xp):
		
		#xp is 1-by-nx
		
		n=self.theano_rng.normal(size=T.shape(self.s_now))
		
		mean_term=T.dot(xp, self.W)/(self.xvar**2)
	...
	def forward_filter_step(self, xp):
		...
		s_samps, prop_terms, prop_means = self.sample_proposal_s(xp)
	...
	def update_params(self, x1, x2, n_samps, lrate):
		...
		sterm=-T.mean(T.sum(T.abs_(self.b*(s2_samps-s_pred)),axis=1)) + T.sum(T.log(self.b))
